...
public class Iko.ValaCode.Writer : Iko.AST.Visitor {
  ...
  public override void visit_system(Iko.AST.System system) {
    ...
    write("static string[%u] x_name = {".printf(system.states.length()));
    for(unowned SList<Iko.AST.State> node = system.states; node != null; node = node.next) {
      write_string_literal(node.data.name);
      if(node.next != null)
        write(",");
    }
    write("};");
    write("static double[%u] x;".printf(system.states.length()));
  }
}
